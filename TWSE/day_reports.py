...
def transform_day_report_row(row: dict[str, Any]) -> dict[str, str]:
    return {
        "company_code": str(row.get("Code", "")).strip(),
        "company_name": str(row.get("Name", "")).strip(),
        "report_period": str(row.get("Date", "")).strip(),
        ...
        "opening_price": str(row.get("OpeningPrice", "")).strip(),
        "highest_price": str(row.get("HighestPrice", "")).strip(),
        "lowest_price": str(row.get("LowestPrice", "")).strip(),
        "closing_price": str(row.get("ClosingPrice", "")).strip(),
        "price_change": str(row.get("Change", "")).strip(),
