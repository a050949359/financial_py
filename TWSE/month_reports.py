...
def fetch_month_reports(
    ...
    if not isinstance(data, list):
        raise ValueError("TWSE payload for month is not a list")
    return data
...
def build_parser() -> argparse.ArgumentParser:
    parser = build_import_parser("TWSE 上市證券月報工具")
...
def main() -> None:
    ...
    imported, db_path = run_import(args, build_import_target(args.config), fetch_month_reports)
    if args.init_schema:
